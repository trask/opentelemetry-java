...
class W3CBaggagePropagatorTest {
  ...
  @ParameterizedTest
  @MethodSource
  void extract_limit_maxEntries(String header, Baggage expectedBaggage) {
    Context result =
        W3CBaggagePropagator.getInstance()
            .extract(Context.root(), ImmutableMap.of("baggage", header), getter);
    assertThat(Baggage.fromContext(result)).isEqualTo(expectedBaggage);
  }
  ...
  static Stream<Arguments> extract_limit_maxEntries() {
    return Stream.of(
        ...
        Arguments.of(baggageHeader(0, 64), baggageWithEntries(0, 64)),
        // One over the limit — only the first 64 extracted
        Arguments.of(baggageHeader(0, 65), baggageWithEntries(0, 64)));
  }
