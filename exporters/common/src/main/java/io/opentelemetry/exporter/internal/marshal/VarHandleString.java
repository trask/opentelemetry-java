...
/**
 * VarHandle-based String field access for Java 9+ as a replacement for sun.misc.Unsafe.
 * This class provides safe and performant access to internal String fields using reflection
...
class VarHandleString {
  ...
  private static Object getStringFieldHandle(String fieldName, Class<?> expectedType) {
    try {
      ...
      Object handle = findVarHandleMethod.invoke(privateLookup, String.class, fieldName, expectedType);
      ...
      if (testValue == null || !expectedType.isAssignableFrom(testValue.getClass())) {
        logger.log(Level.WARNING, "VarHandle for String.{0} returned unexpected type: {1}", 
      ...
      logger.log(Level.FINE, "Cannot create VarHandle for String." + fieldName + 
                 " - likely running on Java 8 or VarHandle not available", exception);
      return null;
    }
  }
