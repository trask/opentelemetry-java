...
public final class DeclarativeConfiguration {
   ...
   *
   * <p>This is used when log record processors are composed, with one processor accepting one or more additional
   * processors as config properties. The {@link ComponentProvider} implementation can call this to
   * configure a delegate {@link LogRecordProcessor} from the {@link DeclarativeConfigProperties}
   * corresponding to a particular config property.
   */
  public static LogRecordProcessor createLogRecordProcessor(DeclarativeConfigProperties genericLogRecordProcessorModel) {
    YamlDeclarativeConfigProperties yamlDeclarativeConfigProperties =
        requireYamlDeclarativeConfigProperties(genericLogRecordProcessorModel);
    LogRecordProcessorModel logRecordProcessorModel =
        MAPPER.convertValue(
            DeclarativeConfigProperties.toMap(yamlDeclarativeConfigProperties), LogRecordProcessorModel.class);
    return createAndMaybeCleanup(
